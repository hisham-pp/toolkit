...
export default function MarkdownHtmlConverter() {
  ...
  const detectAndSetInput = (val: string) => {
    ...
    // Simplified detection: if starts with < and ends with >, assume HTML
  ...
  const convert = async () => {
    ...
    try {
      if (direction === "md-to-html") {
        ...
        // Basic HTML to MD conversion (very simplified)
        // In a real app we might use a dedicated lib like turndown
        toast.info("HTML to Markdown uses heuristic parsing");
        let md = input
          .replace(/<h1>(.*?)<\/h1>/gi, '# $1\n')
          .replace(/<h2>(.*?)<\/h2>/gi, '## $1\n')
          .replace(/<h3>(.*?)<\/h3>/gi, '### $1\n')
      ...
      <div className="flex flex-col h-full gap-8">
        ...
        <div className="flex items-center justify-between bg-[#161618] p-4 border border-zinc-800 rounded-2xl">
          <div className="flex items-center gap-8">
            <div className={cn(
              ...
              direction === "md-to-html" ? "bg-primary/10 border border-primary/20 text-primary" : "bg-zinc-900 border border-zinc-800 text-zinc-500"
            )}>
               <FileText className="w-4 h-4" />
               <span className="text-[10px] font-bold uppercase tracking-widest">Markdown</span>
               {isAutoDetected && direction === "md-to-html" && (
            ...
            <div className={cn(
              "flex items-center gap-3 px-4 py-2 rounded-xl transition-all relative",
              direction === "html-to-md" ? "bg-primary/10 border border-primary/20 text-primary" : "bg-zinc-900 border border-zinc-800 text-zinc-500"
            )}>
               <FileCode className="w-4 h-4" />
               <span className="text-[10px] font-bold uppercase tracking-widest">HTML</span>
               {isAutoDetected && direction === "html-to-md" && (
                 <div className="absolute -top-2 -right-2 bg-primary text-black text-[8px] font-black px-1 rounded animate-bounce">AUTO</div>
               )}
            </div>
          </div>
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 min-h-0">
          <div className="flex flex-col gap-3 group">
             <div className="flex items-center justify-between px-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Source Content</span>
             </div>
          ...
          <div className="flex flex-col gap-3 group">
             <div className="flex items-center justify-between px-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Generated Output</span>
        ...
        <div className="p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-2xl flex items-center gap-4">
           ...
           <p className="text-[10px] text-zinc-500 leading-relaxed font-medium">
             Supports standard CommonMark and GFM. HTML output is clean and semantic. Processing occurs exclusively in your browser session.
           </p>
        </div>
      </div>
  );
